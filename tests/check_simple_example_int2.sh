...
if [ -f "$2".skip ]; then
    ...
    (FUZION_DISABLE_ANSI_ESCAPES=true FUZION_JAVA_OPTIONS="${FUZION_JAVA_OPTIONS="-Xss${FUZION_JAVA_STACK_SIZE=5m}"} ${OPT:-}" $1 -interpreter2 "$2" >tmp_out.txt 2>tmp_err.txt) || true
    sed -i "s|${CURDIR//\\//}/|--CURDIR--/|g" tmp_err.txt
    expout=$2.expected_out
    experr=$2.expected_err
    if [ -f "$2".expected_out_int2 ]; then
        expout=$2.expected_out_int2
    fi
    if [ -f "$2".expected_err_int2 ]; then
        experr=$2.expected_err_int2
    fi
