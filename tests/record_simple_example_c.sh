...
if [ -f "$2".skip ]; then
    ...
    ( (FUZION_DISABLE_ANSI_ESCAPES=true $1 -XmaxErrors=-1 -c ${FUZION_C_BACKEND_OPTIONS:+$FUZION_C_BACKEND_OPTIONS} "$2" -o=testbin && ./testbin) 2>"$2".expected_err_c | head -n 10000) >"$2".expected_out_c || true # tail may result in 141
