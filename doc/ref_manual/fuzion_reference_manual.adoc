...
For redefined feaures, the number of formal value arguments as well as the
number of xref:fuzion_typeparameter[type parameters] must not change compared to the original feature,
otherwise it would not be possible to perform dynamically bind and pass the
...
((Open Type Parameter)): A _open type parameter_ is a xref:fuzion_typeparameter[type parameters] whose declaration is folled by three dots as in `A type ...`.
...
((Type Constraint)):: A _type constraint_ is a type provided in the declaration of a xref:fuzion_typeparameter[type parameter] folling a colon, e.g., `A type : b` which places the constraint `b` on xref:fuzion_typeparameter[type parameter] `A`.
