...
:RULE_SRC: src/dev/flang/ast/AbstractType.java
:RULE_ID: TYPE_SYSTEM_ASSIGNABLE_FROM
include::rule.adoc[]

:RULE_SRC: src/dev/flang/ast/AbstractType.java
:RULE_ID: TYPE_SYSTEM_COMMON_SUPERTYPE
include::rule.adoc[]
...
an xref:fuzion_actual_typeparameter[actual type parameter] or an xref:fuzion_actual_value_argument[actual value argument].
...
an xref:outer_feature[outer feature]. Every feature except for xref:fuzion_typeparameter[type parameters] may define a xref:visibility[visibility]. The default xref:visibility[visibility] is private.
...
The visibility of a feature defines from where this feature can be called.

[cols="1,1,1,1", options="header"]
|===
|Visibility Modifier |Current file |Current module |Other module
...
Furthermore it is possible to control the visibility of the type a feature defines separately.
This is done by first stating the access-visibility colon the type-visibility. These are the possible visibilities
in this case: `private:module`, `private:public`, `module:public`.
