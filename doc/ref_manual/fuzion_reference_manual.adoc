...
[#dynamic_dispatch]
((Dynamic Dispatch)):: Dynamic dispatch is the algorithm that is applied at runtime to determine the
xref:call_desitionation[destination] of a xref:fuzion_call[call] depending on the
xref:dynamic_type[dynanmic type] of the xref:fuzion_target[target] and and xref:feature_name[name] of the
called feature.
...
of that call at runtime results inn the xref:fuzion_instance[instance] that was assigned to that field.
