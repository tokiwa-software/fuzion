...
A _child_ is hence used synonymous to a _heir_.
...
[#fuzion_outer_type]
((Outer Type)):: For any xref:fuzion_type[type] that is based on a feature whose outer feature exists and is not the xref:universe[universe],
there is a corresponding _outer type_ based on a xref:child_feature[child] of the that outer feature.

[#parent_feature]
((Parent)):: A _parent_ of a xref:fuzion_feature[feature] `f` is any feature `g` sucht that `f` xref:inheritance[inherits] from `g`.
A _parent_ is hence used synonymous to an _ancestor_.
...
[#real_child]
((Real Child)):: A _real child_ of a xref:fuzion_feature[feature] `p` is any feature `c` the xref:real_inheritance[inherits really] from `p`.
...
field determined statically by tools like a Fuzion conmpiler.
