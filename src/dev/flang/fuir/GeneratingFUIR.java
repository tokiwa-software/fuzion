    ...
    if (isConst(e) && !dumpResult)
    ...
    else
      {
        if (e instanceof AbstractCall call)
          {
            var currentClazz = _currentClazz;
            var cf = call.calledFeature();
            if (cf.isIntrinsic() && cf.qualifiedName().equals("autogenerated_code_for_OpenType_fold"))
              {
                var fields      = Stream.of(_currentClazz._outer._outer.fields()).filter(c->c.feature().isOpenGenericField()).toList();
                e = call.actuals().get(0);
                ...
                for (var i = 0; i<fields.size(); i++)
                  {
                    ...
                    var ta = _currentClazz.actualTypeParameters()[1];
                    var apply = ta.lookup(new FeatureAndActuals(Types.resolved.f_TypedApplicator_apply,
                                                                new List<>(t)),
                                          FuzionConstants.NO_SELECT,
                                          false /* isInheritanceCall */);
                    var outer = new AbstractCall()
                       {
                         @Override public SourcePosition pos() { return call.pos(); }
                         @Override public List<AbstractType> actualTypeParameters() { return new List<>(); }
                         @Override public AbstractFeature calledFeature() {
                           check(currentClazz.feature().outerRef() != null); return currentClazz.feature().outerRef();
                         }
                         @Override public Expr target() { return new dev.flang.ast.AbstractCurrent(currentClazz._type) {
                             @Override public SourcePosition pos() { return call.pos(); }
                           }; }
                         @Override public List<Expr> actuals() { return new List<>(); }
                         @Override public int select() { return -1; }
                         @Override public boolean isInheritanceCall() { return false; }
                         @Override public AbstractType type() { return currentClazz._outer._type; }
                         @Override public Expr visit(dev.flang.ast.FeatureVisitor v, AbstractFeature outer) { return this; }
                      };
                    var outer2 = new AbstractCall()
                      {
                        @Override public SourcePosition pos() { return call.pos(); }
                        @Override public List<AbstractType> actualTypeParameters() { return new List<>(); }
                        @Override public AbstractFeature calledFeature() {
                          check(currentClazz._outer.feature().outerRef() != null); return currentClazz._outer.feature().outerRef();
                        }
                        @Override public Expr target() { return outer; }
                        @Override public List<Expr> actuals() { return new List<>(); }
                        @Override public int select() { return -1; }
                        @Override public boolean isInheritanceCall() { return false; }
                        @Override public AbstractType type() { return currentClazz._outer._outer._type; }
                        @Override public Expr visit(dev.flang.ast.FeatureVisitor v, AbstractFeature outer) { return this; }
                      };
                    var field = new AbstractCall()
                      {
                        @Override public SourcePosition pos() { return call.pos(); }
                        @Override public List<AbstractType> actualTypeParameters() { return new List<>(); }
                        @Override public AbstractFeature calledFeature() { return f.feature(); }
                        @Override public Expr target() { return outer2; }
                        @Override public List<Expr> actuals() { return new List<>(); }
                        @Override public int select() { return fi; /* f._select; */ }
                        @Override public boolean isInheritanceCall() { return false; }
                        @Override public AbstractType type() { return f.resultClazz()._type; }
                        @Override public Expr visit(dev.flang.ast.FeatureVisitor v, AbstractFeature outer) { return this; }
                      };
                    var fe = e;
                    e = new AbstractCall()
                      {
                        ...
                        @Override public int select() { return -1; }
                        @Override public boolean isInheritanceCall() { return false; }
                        @Override public AbstractType type() { return currentClazz.actualTypeParameters()[fi]._type; }
                        @Override public Expr visit(dev.flang.ast.FeatureVisitor v, AbstractFeature outer) { return this; }
                      };
                  }
              }
