...
public class GeneratingFUIR extends FUIR
{
  ...
  /**
   * For a call to `autogenerated_code_for_Values_Of_Open_Type_fold`/`autogenerated_code_for_Values_Of_Open_Type_zip`,
   * extract the fields that we are operating on.
   *
   * @param currentClazz the current clazz, i.e., the `fold` or `zip` operation
   *
    ...
    _removedIntrinsics_.put
      ("autogenerated_code_for_effect_types_fold",
       (call, currentClazz, fuir) ->
       {
         Expr e = call.actuals().get(0);
         var aa = call.actuals().get(1);
         var type_foldf                = currentClazz.feature();                // effect_types.type_foldf

         if (CHECKS) check
           (type_foldf.outer() == Types.resolved.universe);                     // make sure noone tries to call this in a different context

         // NYI: CLEANUP: This code is a copy from "autogenerated_code_for_Open_Types_fold", should be moved to an auxiliary method!
         for (var cid : fuir._instatedEffectTypes)
           {
             var c = fuir.id2clazz(cid);
             var t = c._type;
             var ta = currentClazz.actualTypeParameters()[1];
             var apply = ta.lookup(new FeatureAndActuals(Types.resolved.f_type_applicator_apply,
                                                         new List<>(t)),
  ...
  final java.util.TreeSet<Integer> _instatedEffectTypes = new TreeSet<>();
  boolean _newInstatedEffectType = false;
  final java.util.TreeSet<Clazz> _effectTypesCode = new TreeSet<>();
  ...
  public int clazzCode(int cl)
  {
    ...
    if (result == NO_SITE && !_lookupDone)
      {
        c.doesNeedCode();
        if (_addingCodeFor != null) throw new Error("adding code for "+c+" not null but "+_addingCodeFor);
        _addingCodeFor = c;
        result = addCode(cl, c);
        if (_addingCodeFor != c) throw new Error("adding code for "+c+"-->"+_addingCodeFor);
        _addingCodeFor = null;
        c._code = result;
      }
    return result;
  ...
  public int effectTypeFromIntrinsic(int cl)
  {
    ...
    if (switch (clazzOriginalName(cl))
          {
          case FuzionConstants.EFFECT_INSTATE_NAME, "effect.type.default0" -> true;
          default -> false;
          }  &&
        !_instatedEffectTypes.contains(res))
      {
        _instatedEffectTypes.add(res);
        _newInstatedEffectType = true;
        for (var c : _effectTypesCode)
          {
            c._code = NO_SITE;
          }
      }
