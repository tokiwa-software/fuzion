...
public class Intrinsix extends ANY implements ClassFileConstants
{
  ...
  static
  {
    ...
    put("fuzion.java.get_static_field0", // TODO: #3852  get_static_field0(T type, clazz Java_Object, field Java_Object, signature fuzion.sys.Pointer) T => intrinsic
        (jvm, si, cc, tvalue, args) ->
        {
          var rc = jvm._fuir.clazzResultClazz(cc);
    ...
    put("fuzion.java.set_static_field0", // TODO:FIXME:
        (jvm, si, cc, tvalue, args) ->
        {
          ...
          var sref0 = jvm._fuir.lookupJavaRef(jvm._fuir.clazzArgClazz(cc, 0));
          var sref1 = jvm._fuir.lookupJavaRef(jvm._fuir.clazzArgClazz(cc, 1));
          var sref2 = jvm._fuir.lookupJavaRef(jvm._fuir.clazzArgClazz(cc, 2));
          var res = args.get(0)
            .andThen(Expr.checkcast(jt))
            .andThen(jvm.getfield(sref0)) // class name as String
            .andThen(Expr.checkcast(JAVA_LANG_STRING))
    ...
    put("fuzion.java.get_field0", // TODO: get_field0(T type, thiz Java_Object, field Java_Object, signature fuzion.sys.Pointer) T => intrinsic
        (jvm, si, cc, tvalue, args) ->
        {
          var rc = jvm._fuir.clazzResultClazz(cc);
