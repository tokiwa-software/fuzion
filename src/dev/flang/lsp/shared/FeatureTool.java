...
public class FeatureTool extends ANY
{
  ...
  {
    require(!feature.baseName().equals(Errors.ERROR_STRING));

    if (feature.baseName().contains(" "))
      {
        var baseNameParts = feature.baseName().split(" ", 2);
        return LexerTool
    ...
    return LexerTool
      .tokensFrom(start)
      .dropWhile(tokenInfo -> !tokenInfo.text().equals(feature.baseName()))
      .map(tokenInfo -> tokenInfo.start())
      .findFirst()
      .get();
  }
  ...
  {
    if (f.baseName().contains(" "))
      {
        return f.baseName().substring(f.baseName().indexOf(" ") + 1);
      }
    return f.baseName();
  }
  ...
  public static boolean isInternal(AbstractFeature af)
  {
    ...
    return af.baseName().startsWith("@")
      ...
      || (af.baseName().matches("a\\d+")
        && af.outer().baseName().equals("call")) // autogenerated
                                                               // lambda args
      || af instanceof Feature f && f.state() == State.ERROR
      || af.baseName().equals(Errors.ERROR_STRING);
  }
