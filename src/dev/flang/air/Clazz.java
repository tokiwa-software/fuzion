...
public class Clazz extends ANY implements Comparable<Clazz>
{
  ...
  private Clazz normalize(AbstractFeature f)
  {
    if (// an outer clazz of value type is not normalized (except for
      ...
      {
        var t = this._type.actualType(f.selfType(), null /* outer */, null /* infix_colons */).asRef();
        return normalize2(t);
      }
  }
  private Clazz normalize2(AbstractType t)
  {
    ...
    if (f.isUniverse())
      {
        return Clazzes.instance.universe.get();
      }
    else
      {
        var normalized = Clazzes.instance.create(t, normalize2(f.outer().selfType()));
   ...
   * which is called for every new Clazz created via Clazzes.instance.create().
   *
    ...
    for (var p: feature().inherits())
      {
        ...
        var t2 = _type.actualType(t1, null /* outer */, null /* infix_colons */);
  ...
  private void lookupIfInstantiated(AbstractFeature f)
  {
    ...
    if (Clazzes.instance.isCalledDynamically(f) &&
        isRef() &&
        isInstantiated())
      {
        for (var ft : Clazzes.instance.calledDynamicallyWithTypePars(f))
          {
  ...
  public Clazz lookup(AbstractFeature f)
  {
    if (PRECONDITIONS) require
      (f != null,
       !this.isVoidType(),
       Clazzes.instance.isUsedAt(f) != null);

    return lookup(f, Clazzes.instance.isUsedAt(f));
  }
   ...
   * the position returned by Clazzes.instance.isUsedAt(fa._f) as the call position.
   *
   * This is not intended for use at runtime, but during analysis of static
   * types or to fill the virtual call table.
   *
  ...
  {
    return lookup(fa, Clazzes.instance.isUsedAt(fa._f));
  }
               ...
               boolean isInheritanceCall)
  {
    ...
    if (select < 0)
    ...
    else
      {
        ...
        innerClazz = select < innerClazzes.length ? innerClazzes[select] : Clazzes.instance.error.get();
      }
    if (innerClazz == null)
      {
        AbstractType t = null;
        ...
        if (t == null)
          {
            innerClazz = Clazzes.instance.error.get();
          }
        else
          {
            t = _type.actualType(t, null /* outer */, null /* infix_colons */);  // e.g., `(Types.get (array f64)).T` -> `array f64`
 ...
 */
  ...
  public String toString()
  {
    ...
    return
      ...
      : (""
         + ((this._outer == Clazzes.instance.universe.get())
            ? ""
  ...
  {
    return this._type.isDirectlyAssignableFrom(other._type, null /* outer */, null /* infix_colons */);
  }
  ...
  /**
   * call Clazzes.instance.findClazzes for all the expressions in f, including all the
        ...
        for (var i = 0; i < n; i++)
          {
            if (i >= cf.valueArguments().size())
            ...
            else
              {
                ...
                argFields[i] = lookup(cfa, Clazzes.instance.isUsedAt(f));
              }
          }
    ...
    if (this._type != Types.t_ADDRESS)
      {
        ...
        for (AbstractFeature ff: _flu.allInnerAndInheritedFeatures(f))
          {
            Clazzes.instance.whenCalledDynamically(ff, () -> lookupIfInstantiated(ff));
          }
      }
  }
    ...
    if (isChoice())
      {
        ...
        for (var t : actualGenerics(feature().choiceGenerics()))
          {
            result.add(Clazzes.instance.clazz(t));
          }
      }
    else
      {
  ...
  public int getChoiceTag(Clazz staticTypeOfValue)
  {
    ...
    for (Clazz g : _choiceGenerics)
      {
        if (g._type.isDirectlyAssignableFrom(staticTypeOfValue._type, null /* outer */, null /* infix_colons */))
          {
            if (CHECKS) check
              (result < 0);
            result = index;
  ...
  void called(HasSourcePosition at)
  {
    ...
    if (at != null &&
        ...
        !_isCalled)
      {
        ...
        if (isCalled())
          {
            var l = Clazzes.instance._whenCalled_.remove(this);
            if (l != null)
              {
                for (var r : l)
                  {
  ...
  private boolean isOuterInstantiated()
  {
    return _outer == null ||
      ...
      _outer == Clazzes.instance.Any.getIfCreated()    ||
  ...
  public Clazz asRef()
  {
    return isRef()
      ? this
      : Clazzes.instance.create(_type.asRef(), _outer);
  }
  ...
  private Clazz inheritedOuterRefClazz(Clazz outer, Expr target, AbstractFeature cf, AbstractFeature f, Clazz result)
  {
    ...
    if (f.outerRef() == cf)
      { // a "normal" outer ref for the outer clazz surrounding this instance or
        ...
        if (outer == null)
          {
            outer = Clazzes.instance.clazz(target, this, new List<>());
          }
        if (CHECKS) check
  ...
  public Clazz typeParameterActualType()
  {
    ...
    return inh == null || ix < 0 || ix >= oag.length ? Clazzes.instance.error.get()
                                                     : oag[ix];
  }
  ...
  Clazz findOuter(AbstractFeature o, HasSourcePosition pos)
  {
    ...
    return i == null ? Clazzes.instance.error.get() : res;
  }
  ...
  Clazz getOuter(AbstractFeature p, HasSourcePosition pos)
  {
    var res =
      ...
      p.isUniverse() ||
      p.outer().isUniverse() ? Clazzes.instance.universe.get()
        ...
        for (int i = 0; i < gs.size(); i++)
          {
            ...
            result[i] = Clazzes.instance.clazz(gi);
          }
      }
    return result;
  }
  ...
  private Clazz determineOuterRef()
  {
    ...
    switch (f.kind())
      ...
      case Routine    :
        {
          var or = f.outerRef();
          if (or != null && Clazzes.instance.isUsed(or))
            {
              result = lookup(or);
            }
          break;
  ...
  Clazz handDown(AbstractType t, int select, List<AbstractCall> inh, HasSourcePosition pos)
  {
    ...
    while (!o.isUniverse() && o != null && oc != null &&
           ...
           )
      {
        ...
        t1 = t1.replace_this_type_by_actual_outer(oc._type, null /* outer */, null /* infix_colons */);
        oc = oc.getOuter(o, pos);
        o = o.outer();
      }

    var t2 = replaceThisType(t1);
    return Clazzes.instance.clazz(t2);
  }
            ...
            if (field.isOpenGenericField())
              {
                ...
                for (var i = 0; i < n; i++)
                  {
                    fields.add(lookup(new FeatureAndActuals(field), i, Clazzes.instance.isUsedAt(field), false));
                  }
              }
            else
              {
  ...
  private Clazz determineAsValue()
  {
    return isRef() && _type != Types.t_ADDRESS
      ? Clazzes.instance.create(_type.asValue(), _outer)
      : this;
  }
