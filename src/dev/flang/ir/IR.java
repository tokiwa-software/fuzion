    ...
    if (e instanceof AbstractAssign a)
    ...
    else if (e instanceof AbstractCall c)
      {
        ...
        for (int i = 0; i < c.actuals().size(); i++)
          {
            var frmlT = c.calledFeature().valueArguments().get(i).resultType();
            if (frmlT.isOpenGeneric())
              {
                var j = 0;
                while (i < c.actuals().size())
                  {
                    var at = frmlT
                      .genericArgument()
                      .replaceOpen(
                        frmlT.genericArgument().feature() == c.calledFeature()
                          ? c.actualTypeParameters()
                          : c.target().type().selfOrConstraint().generics()
                      )
                      .get(j);
                    toStack(l, boxAndTag(c.actuals().get(i), at));
                    i++;
                    j++;
                  }
              }
            else
              {
                var at = c
                    ...
                    .type()
                    .selfOrConstraint()
                    .actualType(c.adjustArgTypeForTarget(frmlT))
                    .applyTypePars(c.calledFeature(), c.actualTypeParameters());
                toStack(l, boxAndTag(c.actuals().get(i), at));
              }
          }
        l.add(c);
