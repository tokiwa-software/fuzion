...
WORKDIR /fuzion
COPY . .
RUN apk add --no-cache bash clang18 git make patch pcre-tools && ln -s /usr/bin/clang-18 /usr/bin/clang && FUZION_REPRODUCIBLE_BUILD="true" PRECONDITIONS="true" POSTCONDITIONS="true" make
